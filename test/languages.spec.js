...
describe('Simlife generator languages', () => {
        ...
        constants.LANGUAGES.forEach((language) => {
            describe(`for ${language.name}`, () => {
                ...
                it('contains 3 needles in global.json', () => {
                    assert.fileContent(
                        `${CLIENT_MAIN_SRC_DIR}i18n/${language.value}/global.json`,
                        '"simlife-needle-menu-add-element": "Simlife will add additional menu entries here (do not translate!)"'
                    );
                    assert.fileContent(
                        `${CLIENT_MAIN_SRC_DIR}i18n/${language.value}/global.json`,
                        '"simlife-needle-menu-add-entry": "Simlife will add additional entities here (do not translate!)"'
                    );
                    assert.fileContent(
                        `${CLIENT_MAIN_SRC_DIR}i18n/${language.value}/global.json`,
                        '"simlife-needle-menu-add-admin-element": "Simlife will add additional menu entries here (do not translate!)"'
                    );
                });
            });
        });
