...
Object.keys(SUB_GENERATORS).forEach((generator) => {
    env.register(require.resolve(`../generators/${generator}`), `${SIMLIFE_NS}:${generator}`);
});

const done = () => {
    logger.info(chalk.green.bold('Congratulations, Simlife execution is complete!'));
};
...
Object.keys(SUB_GENERATORS).forEach((key) => {
    ...
    command.allowUnknownOption()
        ...
        .on('--help', () => {
            ...
            env.run(`${SIMLIFE_NS}:${key} --help`, done);
        });
});
...
if (program.args.length < 1) {
    ...
    logger.info(chalk.yellow('Running default commands'));
    runYoCommand('app', [], {});
}
