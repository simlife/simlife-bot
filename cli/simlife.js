...
if (!semver.satisfies(currentNodeVersion, minimumNodeVersion)) {
    ...
    }\nSimlife requires Node version ${minimumNodeVersion
...
if (process.argv.includes('upgrade') && !process.argv.includes('--prefer-local')) {
    // Prefer global version for `simlife upgrade` to get most recent code
    preferLocal = false;
}
...
function requireCLI(preferLocal) {
    ...
    if (preferLocal) {
        try {
            const localCLI = require.resolve(path.join(process.cwd(), 'node_modules', 'simlife-bot', 'cli', 'cli.js'));
            if (__dirname !== path.dirname(localCLI)) {
                // load local version
                /* eslint-disable import/no-dynamic-require */
                logger.info('Using Simlife version installed locally in current project\'s node_modules');
                require(localCLI);
                return;
            }
        } catch (e) {
    ...
    // load global version
    logger.info('Using Simlife version installed globally');
    require('./cli');
    /* eslint-enable  */
}
