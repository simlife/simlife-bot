...
import { JhiEventManager<% if (queries && queries.length > 0) { %>, JhiAlertService<% } %><% if (fieldsContainBlob) { %>, JhiDataUtils<% } %> } from 'ng-simlife';
...
@Component({
    selector: '<%= simPrefixDashed %>-<%= entityFileName %>-dialog',
...
export class <%= entityAngularName %>DialogComponent implements OnInit {
    ...
    constructor(
        ...
        private simAlertService: JhiAlertService,
        <%_ } _%>
    ...
    private onError(error: any) {
        this.simAlertService.error(error.message, null, null);
    }
    <%_ } _%>
    <%_
    const entitiesSeen = [];
...
@Component({
    selector: '<%= simPrefixDashed %>-<%= entityFileName %>-popup',
    template: ''
})
