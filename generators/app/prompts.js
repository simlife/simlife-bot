...
function askForInsightOptIn() {
    ...
    this.prompt({
        ...
        type: 'confirm',
        name: 'insight',
        message: `May ${chalk.cyan('Simlife')} anonymously report usage statistics to improve the tool over time?`,
        default: true
    }).then((prompt) => {
...
function askForMoreModules() {
    ...
    this.prompt({
        type: 'confirm',
        name: 'installModules',
        message: 'Would you like to install other generators from the Simlife Marketplace?',
        default: false
    }).then((prompt) => {
