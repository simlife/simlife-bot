...
module.exports = class extends PrivateBase {
    /**
     * Get the Simlife configuration from the .yo-rc.json file.
     *
     * @param {string} namespace - namespace of the .yo-rc.json config file. By default: simlife-bot
     */
    getSimlifeAppConfig(namespace = 'simlife-bot') {
    ...
    addElementToMenu(routerName, glyphiconName, enableTranslation, clientFramework) {
        ...
        try {
            ...
            this.log(`${chalk.yellow('\nUnable to find ') + navbarPath + chalk.yellow(' or missing required simlife-needle. Reference to ') + routerName} ${chalk.yellow('not added to menu.\n')}`);
            this.debug('Error:', e);
        }
    }
    ...
    addExternalResourcesToRoot(resources, comment) {
        ...
        try {
            simlifeUtils.rewriteFile({
                file: indexFilePath,
                needle: 'simlife-needle-add-resources-to-root',
                splicable: [resourcesBlock]
            }, this);
        } catch (e) {
            this.log(`${chalk.yellow('\nUnable to find ') + indexFilePath + chalk.yellow(' or missing required simlife-needle. Resources are not added to Simlife app.\n')}`);
            this.debug('Error:', e);
        }
    }
    ...
    addElementToAdminMenu(routerName, glyphiconName, enableTranslation, clientFramework) {
        ...
        try {
            ...
            this.log(`${chalk.yellow('\nUnable to find ') + navbarAdminPath + chalk.yellow(' or missing required simlife-needle. Reference to ') + routerName} ${chalk.yellow('not added to admin menu.\n')}`);
            this.debug('Error:', e);
        }
    }
    ...
    addEntityToWebpack(microserviceName, clientFramework) {
        ...
        simlifeUtils.rewriteFile({
            file: webpackDevPath,
            needle: 'simlife-needle-add-entity-to-webpack',
    ...
    addEntityToMenu(routerName, enableTranslation, clientFramework) {
        ...
        try {
            ...
            this.log(`${chalk.yellow('\nUnable to find ') + entityMenuPath + chalk.yellow(' or missing required simlife-needle. Reference to ') + routerName} ${chalk.yellow('not added to menu.\n')}`);
            this.debug('Error:', e);
        }
    }
        ...
        try {
            if (clientFramework === 'angular1') {
                ...
                simlifeUtils.rewriteFile({
                    ...
                    needle: 'simlife-needle-add-entity-module-import',
                    splicable: [
                ...
                simlifeUtils.rewriteFile({
                    ...
                    needle: 'simlife-needle-add-route-import',
                    splicable: [
            ...
            this.log(`${chalk.yellow('\nUnable to find ') + entityModulePath + chalk.yellow(' or missing required simlife-needle. Reference to ') + entityInstance + entityClass + entityFolderName + entityFileName} ${chalk.yellow(`not added to ${entityModulePath}.\n`)}`);
            this.debug('Error:', e);
        }
    }
    ...
    addAdminToModule(appName, adminAngularName, adminFolderName, adminFileName, enableTranslation, clientFramework) {
        ...
        try {
            ...
            simlifeUtils.rewriteFile({
                ...
                needle: 'simlife-needle-add-admin-module-import',
                splicable: [
                    this.stripMargin(importStatement)
                ]
            }, this);
            ...
            simlifeUtils.rewriteFile({
                file: adminModulePath,
                needle: 'simlife-needle-add-admin-module',
                splicable: [
                    this.stripMargin(`|${appName}${adminAngularName}Module,`)
                ]
            }, this);
        } catch (e) {
            this.log(`${chalk.yellow('\nUnable to find ') + appName + chalk.yellow(' or missing required simlife-needle. Reference to ') + adminAngularName + adminFolderName + adminFileName + enableTranslation + clientFramework} ${chalk.yellow(`not added to ${adminModulePath}.\n`)}`);
            this.debug('Error:', e);
        }
    }
    ...
    addElementTranslationKey(key, value, language) {
        ...
        try {
            simlifeUtils.rewriteFile({
                file: fullPath,
                needle: 'simlife-needle-menu-add-element',
                splicable: [
                    `"${key}": "${_.startCase(value)}",`
                ]
            }, this);
        } catch (e) {
            this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required simlife-needle. Reference to ') + language + chalk.yellow(' not added as a new entity in the menu.\n'));
            this.debug('Error:', e);
        }
    }
    ...
    addAdminElementTranslationKey(key, value, language) {
        ...
        try {
            simlifeUtils.rewriteFile({
                file: fullPath,
                needle: 'simlife-needle-menu-add-admin-element',
                splicable: [
                    `"${key}": "${_.startCase(value)}",`
                ]
            }, this);
        } catch (e) {
            this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required simlife-needle. Reference to ') + language + chalk.yellow(' not added as a new entry in the admin menu.\n'));
            this.debug('Error:', e);
        }
    }
    ...
    addEntityTranslationKey(key, value, language) {
        ...
        try {
            simlifeUtils.rewriteFile({
                file: fullPath,
                needle: 'simlife-needle-menu-add-entry',
                splicable: [
                    `"${key}": "${_.startCase(value)}",`
                ]
            }, this);
    ...
    addGlobalTranslationKey(key, value, language) {
        const fullPath = `${CLIENT_MAIN_SRC_DIR}i18n/${language}/global.json`;
        try {
            simlifeUtils.rewriteJSONFile(fullPath, (jsonObj) => {
                jsonObj[key] = value;
            }, this);
        } catch (e) {
    ...
    /**
     * get all the languages supported by Simlife
     */
    ...
    /**
     * check if a language is supported by Simlife
    ...
    /**
     * get all the languages options supported by Simlife
     */
    ...
    addAngularModule(appName, angularName, folderName, fileName, enableTranslation, clientFramework) {
        ...
        try {
            ...
            simlifeUtils.rewriteFile({
                file: modulePath,
                needle: 'simlife-needle-angular-add-module-import',
                splicable: [
                    this.stripMargin(importStatement)
                ]
            }, this);
            ...
            simlifeUtils.rewriteFile({
                file: modulePath,
                needle: 'simlife-needle-angular-add-module',
                splicable: [
                    this.stripMargin(`|${appName}${angularName}Module,`)
                ]
            }, this);
        } catch (e) {
            this.log(`${chalk.yellow('\nUnable to find ') + appName + chalk.yellow(' or missing required simlife-needle. Reference to ') + angularName + folderName + fileName + enableTranslation + clientFramework} ${chalk.yellow(`not added to ${modulePath}.\n`)}`);
            this.debug('Error:', e);
        }
    }
    ...
    addAngularJsInterceptor(interceptorName) {
        ...
        try {
            simlifeUtils.rewriteFile({
                file: fullPath,
                needle: 'simlife-needle-angularjs-add-interceptor',
                splicable: [
                    `$httpProvider.interceptors.push('${interceptorName}');`
                ]
            }, this);
        } catch (e) {
            this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required simlife-needle. Interceptor not added to Simlife app.\n'));
            this.debug('Error:', e);
        }
    }
    ...
    addEntryToCache(entry, packageFolder, cacheProvider) {
        try {
            ...
            if (cacheProvider === 'ehcache') {
                simlifeUtils.rewriteFile({
                    file: cachePath,
                    needle: 'simlife-needle-ehcache-add-entry',
                ...
                simlifeUtils.rewriteFile({
                    file: cachePath,
                    needle: 'simlife-needle-infinispan-add-entry',
    ...
    addChangelogToLiquibase(changelogName) {
        this.addLiquibaseChangelogToMaster(changelogName, 'simlife-needle-liquibase-add-changelog');
    }
    ...
    addConstraintsChangelogToLiquibase(changelogName) {
        this.addLiquibaseChangelogToMaster(changelogName, 'simlife-needle-liquibase-add-constraints-changelog');
    }
    ...
    addLiquibaseChangelogToMaster(changelogName, needle) {
        ...
        try {
            ...
            this.log(`${chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required simlife-needle. Reference to ') + changelogName}.xml ${chalk.yellow('not added.\n')}`);
            this.debug('Error:', e);
        }
    }
    ...
    addColumnToLiquibaseEntityChangeset(filePath, content) {
        try {
            simlifeUtils.rewriteFile({
                file: filePath,
                needle: 'simlife-needle-liquibase-add-column',
                splicable: [
                    content
                ]
            }, this);
        } catch (e) {
            this.log(chalk.yellow('\nUnable to find ') + filePath + chalk.yellow(' or missing required simlife-needle. Column not added.\n') + e);
            this.debug('Error:', e);
        }
    }
    ...
    addChangesetToLiquibaseEntityChangelog(filePath, content) {
        try {
            simlifeUtils.rewriteFile({
                file: filePath,
                needle: 'simlife-needle-liquibase-add-changeset',
                splicable: [
                    content
                ]
            }, this);
        } catch (e) {
            this.log(chalk.yellow('\nUnable to find ') + filePath + chalk.yellow(' or missing required simlife-needle. Changeset not added.\n') + e);
            this.debug('Error:', e);
        }
    }
    ...
    addMainCSSStyle(isUseSass, style, comment) {
        ...
        try {
            simlifeUtils.rewriteFile({
                file: fullPath,
                needle: 'simlife-needle-css-add-main',
                splicable: [
                    styleBlock
                ]
            }, this);
        } catch (e) {
            this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required simlife-needle. Style not added to Simlife app.\n'));
            this.debug('Error:', e);
        }
    }
    ...
    addMainSCSSStyle(style, comment) {
        ...
        try {
            simlifeUtils.rewriteFile({
                file: fullPath,
                needle: 'simlife-needle-scss-add-main',
                splicable: [
                    styleBlock
                ]
            }, this);
    ...
    addVendorSCSSStyle(style, comment) {
        ...
        try {
            simlifeUtils.rewriteFile({
                file: fullPath,
                needle: 'simlife-needle-scss-add-vendor',
                splicable: [
                    styleBlock
                ]
            }, this);
    ...
    copyExternalAssetsInWebpack(sourceFolder, targetFolder) {
        ...
        try {
            simlifeUtils.rewriteFile({
                file: webpackDevPath,
                needle: 'simlife-needle-add-assets-to-webpack',
                splicable: [
                    assetBlock
                ]
            }, this);
        } catch (e) {
            this.log(chalk.yellow('\nUnable to find ') + webpackDevPath + chalk.yellow(' or missing required simlife-needle. Resource path not added to Simlife app.\n'));
            this.debug('Error:', e);
        }
    }
    ...
    addMavenDependencyManagement(groupId, artifactId, version, type, scope, other) {
        ...
        try {
            ...
            simlifeUtils.rewriteFile({
                file: fullPath,
                needle: 'simlife-needle-maven-add-dependency-management',
                splicable: [
                    dependency
                ]
            }, this);
    ...
    addMavenProperty(name, value) {
        ...
        try {
            ...
            simlifeUtils.rewriteFile({
                file: fullPath,
                needle: 'simlife-needle-maven-property',
                splicable: [
                    property
                ]
            }, this);
        } catch (e) {
            this.log(`${chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required simlife-needle. Reference to ')}maven property (name: ${name}, value:${value})${chalk.yellow(' not added.\n')}`);
            this.debug('Error:', e);
        }
    }
    ...
    addMavenPlugin(groupId, artifactId, version, other) {
        ...
        try {
            ...
            simlifeUtils.rewriteFile({
                file: fullPath,
                needle: 'simlife-needle-maven-add-plugin',
                splicable: [
                    plugin
                ]
            }, this);
        } catch (e) {
            this.log(`${chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required simlife-needle. Reference to ')}maven plugin (groupId: ${groupId}, artifactId:${artifactId}, version:${version})${chalk.yellow(' not added.\n')}`);
            this.debug('Error:', e);
        }
    }
    ...
    addMavenProfile(profileId, other) {
        ...
        try {
            ...
            simlifeUtils.rewriteFile({
                file: fullPath,
                needle: 'simlife-needle-maven-add-profile',
                splicable: [
                    profile
                ]
            }, this);
        } catch (e) {
            this.log(`${chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required simlife-needle. Reference to ')}maven profile (id: ${profileId})${chalk.yellow(' not added.\n')}`);
            this.debug('Error:', e);
        }
    }
    ...
    addGradlePlugin(group, name, version) {
        ...
        try {
            simlifeUtils.rewriteFile({
                file: fullPath,
                needle: 'simlife-needle-gradle-buildscript-dependency',
                splicable: [
                    `classpath '${group}:${name}:${version}'`
                ]
            }, this);
        } catch (e) {
            this.log(`${chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required simlife-needle. Reference to ')}classpath: ${group}:${name}:${version}${chalk.yellow(' not added.\n')}`);
            this.debug('Error:', e);
        }
    }
    ...
    addGradleDependencyManagement(scope, group, name, version) {
        ...
        try {
            simlifeUtils.rewriteFile({
                file: fullPath,
                needle: 'simlife-needle-gradle-dependency-management',
                splicable: [
                    `${scope} "${dependency}"`
                ]
            }, this);
    ...
    applyFromGradleScript(name) {
        ...
        try {
            simlifeUtils.rewriteFile({
                file: fullPath,
                needle: 'simlife-needle-gradle-apply-from',
                splicable: [
    ...
    copyTemplate(source, dest, action, generator, opt = {}, template) {
        ...
        case 'stripHtml':
            regex = new RegExp([
                /( (data-t|simT)ranslate="([a-zA-Z0-9 +{}'_](\.)?)+")/, // data-translate or simTranslate
            ...
            simlifeUtils.copyWebResource(source, dest, regex, 'html', _this, opt, template);
            break;
        case 'stripJs':
            regex = new RegExp([
    ...
    registerModule(npmPackageName, hookFor, hookType, callbackSubGenerator, description) {
        try {
            ...
            let error;
            let duplicate;
            const moduleName = _.startCase(npmPackageName.replace(`${GENERATOR_SIMLIFE}-`, ''));
            ...
            const moduleConfig = {
                ...
                npmPackageName,
                description: description || `A Simlife module to generate ${moduleName}`,
                hookFor,
                hookType,
                generatorCallback
            };
            try {
                ...
                error = true;
                this.log(chalk.red('The Simlife module configuration file could not be read!'));
            ...
            this.log(`\n${chalk.bold.red('Could not add simlife module configuration')}`);
            this.debug('Error:', err);
        }
    }
    ...
    updateEntityConfig(file, key, value) {
        try {
            ...
            this.log(chalk.red('The Simlife entity configuration file could not be read!') + err);
            this.debug('Error:', err);
        }
    }
    ...
    getExistingEntities() {
        ...
        if (!shelljs.test('-d', SIMLIFE_CONFIG_DIR)) {
            return entities;
        }
    ...
    /**
     * Check if the Simlife version used to generate an existing project is less than the passed version argument
     *
    ...
    isJsimlifeVersionLessThan(version) {
        ...
        return semver.lt(simlifeVersion, version);
    }
    ...
    /**
     * get a table name in Simlife preferred style.
     *
    ...
    /**
     * get a table column name in Simlife preferred style.
     *
    ...
    /**
     * get a table column names plural form in Simlife preferred style.
     *
     * @param {string} value - table column name string
     */
    ...
    /**
     * get a table name for joined tables in Simlife preferred style.
     *
    ...
    /**
     * get a constraint name for tables in Simlife preferred style.
     *
     * @param {string} entityName - name of the entity
     * @param {string} relationshipName - name of the related entity
     * @param {string} prodDatabaseType - database type
    ...
    /**
     * Checks if there is a newer Simlife version available.
     */
    checkForNewVersion() {
        try {
            const done = this.async();
            shelljs.exec(`npm show ${GENERATOR_SIMLIFE} version`, { silent: true }, (code, stdout, stderr) => {
                if (!stderr && semver.lt(packagejs.version, stdout)) {
                    ...
                    if (this.useYarn) {
                        ...
                        this.log(chalk.yellow(`  Run ${chalk.magenta(`npm install -g ${GENERATOR_SIMLIFE}`)} to update.\n`));
                    }
                    this.log(chalk.yellow(' ______________________________________________________________________________\n'));
                }
                done();
