...
module.exports = class extends BaseGenerator {
    ...
    get initializing() {
        return {
            sayHello() {
                this.log(chalk.white(`${chalk.bold('🐳')}  Welcome to the Simlife Docker Compose Sub-Generator ${chalk.bold('🐳')}`));
            ...
            setupServerConsts() {
                ...
                this.DOCKER_SIMLIFE_CONSOLE = constants.DOCKER_SIMLIFE_CONSOLE;
                this.DOCKER_SIMLIFE_CURATOR = constants.DOCKER_SIMLIFE_CURATOR;
                this.DOCKER_SIMLIFE_ELASTICSEARCH = constants.DOCKER_SIMLIFE_ELASTICSEARCH;
                this.DOCKER_SIMLIFE_LOGSTASH = constants.DOCKER_SIMLIFE_LOGSTASH;
                this.DOCKER_SIMLIFE_IMPORT_DASHBOARDS = constants.DOCKER_SIMLIFE_IMPORT_DASHBOARDS;
                this.DOCKER_SIMLIFE_ZIPKIN = constants.DOCKER_SIMLIFE_ZIPKIN;
            ...
            setAppsYaml() {
                ...
                this.appsFolders.forEach((appsFolder, index) => {
                    ...
                    if (appConfig.applicationType === 'monolith' && this.monitoring === 'elk') {
                        yamlConfig.environment.push('SIMLIFE_LOGGING_LOGSTASH_ENABLED=true');
                        yamlConfig.environment.push('SIMLIFE_LOGGING_LOGSTASH_HOST=simlife-logstash');
                        yamlConfig.environment.push('SIMLIFE_METRICS_LOGS_ENABLED=true');
                        yamlConfig.environment.push('SIMLIFE_METRICS_LOGS_REPORT_FREQUENCY=60');
                    }

                    if (this.monitoring === 'prometheus') {
                        yamlConfig.environment.push('SIMLIFE_METRICS_PROMETHEUS_ENABLED=true');
                        yamlConfig.environment.push('SIMLIFE_METRICS_PROMETHEUS_ENDPOINT=/prometheusMetrics');
                    }

                    if (this.serviceDiscoveryType === 'eureka') {
                        // Set the Simlife Registry password
                        yamlConfig.environment.push(`SIMLIFE_REGISTRY_PASSWORD=${this.adminPassword}`);
                    }
