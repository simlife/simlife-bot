...
module.exports = class extends Generator {
    ...
    updateLanguagesInLanguageConstant(languages) {
        ...
        try {
            ...
            simlifeUtils.replaceContent({
                file: fullPath,
                pattern: /\.constant.*LANGUAGES.*\[([^\]]*simlife-needle-i18n-language-constant[^\]]*)\]/g,
                content
            }, this);
        } catch (e) {
            this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required simlife-needle. LANGUAGE constant not updated with languages: ') + languages + chalk.yellow(' since block was not found. Check if you have enabled translation support.\n'));
            this.debug('Error:', e);
        }
    }
    ...
    updateLanguagesInLanguageConstantNG2(languages) {
        ...
        try {
            ...
            simlifeUtils.replaceContent({
                file: fullPath,
                pattern: /export.*LANGUAGES.*\[([^\]]*simlife-needle-i18n-language-constant[^\]]*)\];/g,
                content
            }, this);
        } catch (e) {
            this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required simlife-needle. LANGUAGE constant not updated with languages: ') + languages + chalk.yellow(' since block was not found. Check if you have enabled translation support.\n'));
    ...
    updateLanguagesInLanguagePipe(languages) {
        ...
        try {
            ...
            simlifeUtils.replaceContent({
                file: fullPath,
                pattern: /{\s*('[a-z-]*':)?([^=]*simlife-needle-i18n-language-key-pipe[^;]*)\};/g,
                content
            }, this);
        } catch (e) {
            this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required simlife-needle. Language pipe not updated with languages: ') + languages + chalk.yellow(' since block was not found. Check if you have enabled translation support.\n'));
            this.debug('Error:', e);
        }
    }
     ...
     */
    getDefaultAppName() {
        return (/^[a-zA-Z0-9_]+$/.test(path.basename(process.cwd()))) ? path.basename(process.cwd()) : 'simlife';
    }
    ...
    formatAsClassJavadoc(text) {
        return simlifeUtils.getJavadoc(text, 0);
    }
    ...
    formatAsFieldJavadoc(text) {
        return simlifeUtils.getJavadoc(text, 4);
    }
    ...
    /**
     * get hibernate SnakeCase in Simlife preferred style.
     *
     ...
     * @returns hibernate SnakeCase in Simlife preferred style
     */
...
2. If you are using an HTTP proxy, try this command: ${chalk.yellow('git config --global url."https://".insteadOf git://')}`);
    ...
    getUaaAppName(input) {
        ...
        if (shelljs.test('-f', fromPath)) {
            ...
            if (fileData && fileData['simlife-bot']) {
                return fileData['simlife-bot'];
            } return false;
        }
        return false;
    }
