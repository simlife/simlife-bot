...
module.exports = class extends BaseGenerator {
    get initializing() {
        return {
            sayHello() {
                this.log(chalk.white('Welcome to the Simlife Information Sub-Generator\n'));
            },

            checkSimlife() {
                const done = this.async();
                console.log('##### **Simlife Version(s)**');
                shelljs.exec('npm list simlife-bot', { silent: true }, (err, stdout, stderr) => {
                    if (stdout) {
                        console.log(`\n\`\`\`\n${stdout}\`\`\`\n`);
                    }
                    done();
            ...
            displayConfiguration() {
                ...
                result = result.replace(/"rememberMeKey": ".*"/g, '"rememberMeKey": "replaced-by-simlife-info"');
                result = result.replace(/"jwtSecretKey": ".*"/g, '"jwtSecretKey": "replaced-by-simlife-info"');
                console.log('\n##### **Simlife configuration, a `.yo-rc.json` file generated in the root folder**\n');
            ...
            displayEntities() {
                const done = this.async();
                console.log('\n##### **JDL for the Entity configuration(s) `entityName.json` files generated in the `.simlife` directory**\n');
