...
import { JhiLanguageService, JhiDataUtils, JhiDateUtils, JhiEventManager, JhiAlertService, JhiParseLinks } from 'ng-simlife';

import { MockLanguageService, MockLanguageHelper } from './helpers/mock-language.service';
import { JhiLanguageHelper, Principal, AccountService<% if (authenticationType !== 'oauth2') { %>, LoginModalService<% } %><% if (websocket === 'spring-websocket') { %>, <%=simPrefixCapitalized%>TrackerService<% } %> } from '../../../main/webapp/app/shared';
<%_ } else { _%>
...
@NgModule({
    providers: [
        ...
        {
            provide: <%=simPrefixCapitalized%>TrackerService,
            useValue: null
        },
        <%_ } _%>
        {
