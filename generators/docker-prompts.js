...
function askForGatewayType() {
    ...
    const prompts = [{
        ...
        choices: [
            {
                value: 'zuul',
                name: 'Simlife gateway based on Netflix Zuul'
            },
            {
                value: 'traefik',
...
function askForPath() {
    ...
    this.prompt(prompts).then((props) => {
        ...
        for (let i = this.appsFolders.length - 1; i >= 0; i--) {
            if (this.appsFolders[i] === 'simlife-registry' || this.appsFolders[i] === 'registry') {
...
function askForMonitoring() {
    ...
    const prompts = [{
        ...
        choices: [
            ...
            {
                value: 'elk',
                name: (this.composeApplicationType === 'monolith') ? 'Yes, for logs and metrics with the Simlife Console (based on ELK)' : 'Yes, for logs and metrics with the Simlife Console (based on ELK and Zipkin)'
            },
            {
                value: 'prometheus',
...
function askForConsoleOptions() {
    ...
    const prompts = [{
        type: 'checkbox',
        name: 'consoleOptions',
        message: 'You have selected the Simlife Console which is based on the ELK stack and additional technologies, which one do you want to use ?',
        choices: [
            {
                value: 'curator',
    ...
    if (this.composeApplicationType === 'microservice') {
        prompts[0].choices.push({
            value: 'zipkin',
            name: 'Zipkin, for distributed tracing (only compatible with Simlife >= v4.2.0)'
        });
    }
    this.prompt(prompts).then((props) => {
...
function askForServiceDiscovery() {
    ...
    if (serviceDiscoveryEnabledApps.every(app => app.serviceDiscoveryType === 'consul')) {
        ...
        this.log(chalk.green('Simlife registry detected as the service discovery and configuration provider used by your apps'));
        done();
    } else {
...
function askForAdminPassword() {
    ...
    const prompts = [{
        type: 'input',
        name: 'adminPassword',
        message: 'Enter the admin password used to secure the Simlife Registry',
        default: 'admin',
        ...
        if (file.isDirectory()) {
            if ((shelljs.test('-f', `${destinationPath}/${file.name}/.yo-rc.json`))
                ...
                try {
                    ...
                    if ((fileData['simlife-bot'].baseName !== undefined)
                        && ((composeApplicationType === undefined)
                            || (composeApplicationType === fileData['simlife-bot'].applicationType)
                            || ((composeApplicationType === 'microservice') && (fileData['simlife-bot'].applicationType === 'gateway'))
                            || ((composeApplicationType === 'microservice') && (fileData['simlife-bot'].applicationType === 'uaa')))) {
