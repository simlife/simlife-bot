...
oc process -f <%-directoryPath%>/ocp/registry/simlife-registry.yml | oc apply -f -
...
oc process -f <%-directoryPath%>/ocp/monitoring/simlife-monitoring.yml | oc apply -f -
<%_ } _%> <%_ if (monitoring === 'prometheus') { _%>
oc process -f <%-directoryPath%>/ocp/monitoring/simlife-metrics.yml | oc apply -f -
