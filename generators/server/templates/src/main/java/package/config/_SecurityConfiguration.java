...
import io.github.simlife.security.*;
<%_ } _%>
...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    public SecurityConfiguration(<%_ if (authenticationType !== 'oauth2' && !skipUserManagement) { _%>AuthenticationManagerBuilder authenticationManagerBuilder, UserDetailsService userDetailsService<%_ } _%><%_ if (authenticationType === 'session') { _%>,
        SimlifeProperties jSimlifeProperties, RememberMeServices rememberMeServices<%_ } if (authenticationType === 'jwt') { _%><%_ if (!skipUserManagement) { _%>,<%_ } _%>TokenProvider tokenProvider<%_ } _%>
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        .and()
            ...
            .key(jSimlifeProperties.getSecurity().getRememberMe().getKey())
        .and()
            .formLogin()
