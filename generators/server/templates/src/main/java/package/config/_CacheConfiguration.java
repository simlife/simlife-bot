...
import io.github.simlife.config.jcache.BeanClassLoaderAwareJCacheRegionFactory;
    <%_ } _%>
...
public class CacheConfiguration {
    ...
    public CacheConfiguration(SimlifeProperties jSimlifeProperties) {
        <%_ if (enableHibernateCache) { _%>
        BeanClassLoaderAwareJCacheRegionFactory.setBeanClassLoader(this.getClass().getClassLoader());
        <%_ } _%>
        SimlifeProperties.Cache.Ehcache ehcache =
            jSimlifeProperties.getCache().getEhcache();
    ...
    public JCacheManagerCustomizer cacheManagerCustomizer() {
        return cm -> {
            ...
            cm.createCache(<%=packageName%>.domain.SocialUserConnection.class.getName(), jcacheConfiguration);
                ...
                <%_ } _%>
            <%_ } _%>
            // simlife-needle-ehcache-add-entry
        };
    }
    <%_ } _%>
    <%_ if (cacheProvider === 'hazelcast') { _%>
    ...
    @Bean
    public HazelcastInstance hazelcastInstance(SimlifeProperties jSimlifeProperties) {
        ...
        config.setManagementCenterConfig(initializeDefaultManagementCenterConfig(jSimlifeProperties));
        <%_ if (enableHibernateCache) { _%>
        config.getMapConfigs().put("<%=packageName%>.domain.*", initializeDomainMapConfig(jSimlifeProperties));
        <%_ } _%>
        return Hazelcast.newHazelcastInstance(config);
    }

    private ManagementCenterConfig initializeDefaultManagementCenterConfig(SimlifeProperties jSimlifeProperties) {
        ManagementCenterConfig managementCenterConfig = new ManagementCenterConfig();
        managementCenterConfig.setEnabled(jSimlifeProperties.getCache().getHazelcast().getManagementCenter().isEnabled());
        managementCenterConfig.setUrl(jSimlifeProperties.getCache().getHazelcast().getManagementCenter().getUrl());
        managementCenterConfig.setUpdateInterval(jSimlifeProperties.getCache().getHazelcast().getManagementCenter().getUpdateInterval());
    ...
    private MapConfig initializeDomainMapConfig(SimlifeProperties jSimlifeProperties) {
        MapConfig mapConfig = new MapConfig();
        mapConfig.setTimeToLiveSeconds(jSimlifeProperties.getCache().getHazelcast().getTimeToLiveSeconds());
        return mapConfig;
    }
    <%_ } _%>
    <%_ } _%>
     ...
     * <p>
     * If the Simlife Registry is enabled, then the host list will be populated
     * from Eureka.
     *
     * <p>
     * If the Simlife Registry is not enabled, host discovery will be based on
     ...
     * <i>simlife.cache.inifnispan.config-file</i>
     *
     * <p>
     * If the Simlife Registry is not defined, you have the choice of 'config-file'
     ...
     */
    @Bean
    public InfinispanGlobalConfigurer globalConfiguration(SimlifeProperties jSimlifeProperties) {
        ...
        <%_ if (serviceDiscoveryType === 'eureka') { _%>
            if(this.registration == null) { // if registry is not defined, use native discovery
                ...
                return () -> GlobalConfigurationBuilder
                    ...
                    .addProperty("configurationFile", jSimlifeProperties.getCache().getInfinispan().getConfigFile())
                    .clusterName("infinispan-<%=baseName%>-cluster").globalJmxStatistics()
                    .enabled(jSimlifeProperties.getCache().getInfinispan().isStatsEnabled())
     ...
     */
    @Bean
    public InfinispanCacheConfigurer cacheConfigurer(SimlifeProperties jSimlifeProperties) {
        log.info("Defining {} configuration", "app-data for local, replicated and distributed modes");
        final SimlifeProperties.Cache.Infinispan cacheInfo = jSimlifeProperties.getCache().getInfinispan();
     ...
     */
    @Bean
    public JCacheManager getJCacheManager(EmbeddedCacheManager cacheManager, SimlifeProperties jSimlifeProperties){
        return new InfinispanJCacheManager(Caching.getCachingProvider().getDefaultURI(), cacheManager,
            Caching.getCachingProvider(), jSimlifeProperties);
    }
    ...
    class InfinispanJCacheManager extends JCacheManager {
        ...
        public InfinispanJCacheManager(URI uri, EmbeddedCacheManager cacheManager, CachingProvider provider,
            ...
            registerPredefinedCache(<%=packageName%>.domain.SocialUserConnection.class.getName(), new JCache<Object, Object>(
                ...
                <%_ } _%>
            <%_ } _%>
            // simlife-needle-infinispan-add-entry
            if (jSimlifeProperties.getCache().getInfinispan().isStatsEnabled()) {
        ...
        <%_ if(serviceDiscoveryType === 'eureka') { _%>
    /**
     * TCP channel with the host details populated from the Simlife Registry.
     * <p>
