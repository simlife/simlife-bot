 ...
 */<% if (databaseType === 'sql') { %>
@Entity
@Table(name = "<%= simTablePrefix %>_user")<% } %>
...
@org.springframework.data.mongodb.core.mapping.Document(collection = "<%= simTablePrefix %>_user")<% } %><% if (databaseType === 'couchbase') { %>
...
<%_ } _%>
    ...
    @ManyToMany
    @JoinTable(
        name = "<%= simTablePrefix %>_user_authority",
