 ...
 */<% if (databaseType === 'sql') { %>
@Entity
@Table(name = "<%= simTablePrefix %>_persistent_audit_event")<% } %><% if (databaseType === 'mongodb') { %>
@Document(collection = "<%= simTablePrefix %>_persistent_audit_event")<% } %><% if (databaseType === 'couchbase') { %>
@Document<% } %>
...
<% if (databaseType === 'sql') { %>
    ...
    @Column(name = "value")
    @CollectionTable(name = "<%= simTablePrefix %>_persistent_audit_evt_data", joinColumns=@JoinColumn(name="event_id"))<% } %>
